...
function createBase58Indexes() {
    const indexes = new Int16Array(128);
    indexes.fill(-1);
    for (let index = 0; index < BASE58_ALPHABET.length; index++) {
...
export function encodeBase58(bytes) {
    ...
    const digits = [0];
    for (let byteIndex = leadingZeroCount; byteIndex < bytes.length; byteIndex++) {
...
export function decodeBase58(encoded) {
    ...
    const bytes = [0];
    for (let characterIndex = leadingOneCount; characterIndex < encoded.length; characterIndex++) {
        ...
        if (alphabetIndex < 0) {
            throw new Error("Invalid base58 character at index " + String(characterIndex));
        }
...
export function decodePublicKey(publicKey) {
    ...
    if (bytes.length !== 32) {
        throw new Error("Public key must contain 32 bytes; received " + String(bytes.length));
    }
    return bytes;
}
